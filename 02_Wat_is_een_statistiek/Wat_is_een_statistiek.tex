...
Statistiek gaat dus niet alleen over het berekenen van cijfers, ook wel \emph{data} genoemd. Statistiek gaat ook over het beheer van allerhande informatie die bij de cijfers hoort. Deze informatie noemen we de \emph{attributen} van de cijfers of \emph{metadata}. Een \emph{statistiek} bestaat dus altijd uit twee onlosmakelijke elementen: het cijfer zelf én de attributen (zie Figuur \ref{fig:statistiek}). Beiden zijn even belangrijk en vormen één geheel \parencite{Wild2018What}. In deze paragraaf gaan we dieper in op verschillende belangrijke attributen van een statistiek. 

\begin{figure}
\caption{Op het laagste niveau bestaat een statistiek steeds uit een cijfer en attributen van dat cijfer.}
\label{fig:statistiek}
\begin{center}
\begin{tikzpicture}[
	x=\linewidth,
	y=15mm,
	concept/.style={font=\Large,text=deepslateblue},
	label/.style={anchor=north,text=white,font=\large},
	]
\fill[lightskyblue] (0,.7) rectangle (1,-1); 	
\node[concept] (A) at (.25,0) {Cijfer};
\node[label] at (A.south) {Data};
\node[concept] (B) at (.75,0) {Attributen};
\node[label] at (B.south) {Metadata};
\draw[latex-latex,very thick,bend left=20,softsteelblue] (A.east) to (B.west);
\end{tikzpicture}
\end{center}
\end{figure}	
...
Het eerste belangrijke attribuut van een statistiek is uiteraard de betekenis van het cijfer. Die betekenis wordt vormgegeven door de \emph{conceptuele definitie} van de statistiek \parencite{neuman2014Social}. Een conceptuele definitie is een beschrijving van wat de statistiek als een begrip of concept inhoudt, op een abstract en theoretisch niveau. De conceptuele definitie van het cijfer 525\,935 is bijvoorbeeld ``het aantal inwoners van de gemeente Antwerpen in 2019''. 
...
\begin{tikzpicture}[
	...
	arr/.style={-latex,white}
	]
\fill[lightskyblue]	(0,-1) rectangle (1,1);
...
Wanneer er problemen ontstaan rond tijdigheid en punctualiteit van de statistiek is dit doorgaans omwille van een verkeerd gekozen operationele definitie om de statistiek te meten. Zo kan een operationele definitie inhoudelijk aansluiten bij een gebruikersbehoefte, maar als de berekening te lang duurt en de statistiek daardoor te laat beschikbaar is, verlaagt dit alsnog de kwaliteit.
...
Opnieuw worden problemen in verband met vergelijkbaarheid veroorzaakt door slecht gekozen conceptuele en operationele definities. Hoe sterker de definities van twee cijfers van elkaar afwijken, hoe minder deze cijfers vergelijkbaar zijn. Merk op dat streven naar vergelijkbaarheid en streven naar nauwkeurigheid in sommige situaties tegengestelde doelen kunnen zijn. De nauwkeurigheid van een statistiek kan in bepaalde situaties bijvoorbeeld verbeterd worden door nieuwe dataverzamelings- en -verwerkingsmethoden. Zo'n nieuwe methoden kunnen echter tot gevolg hebben dat de statistiek niet meer vergelijkbaar is met eerder gepubliceerde cijfers. De statistiekdienst zal in zo'n situatie een gedragen evenwicht moeten vinden tussen nauwkeurigheid en vergelijkbaarheid. 
...
\input{tables/voorbeeldcijfertabel.tex}
\end{table}
...
De \emph{operationele definitie} van een cijfertabel beschrijft hoe de cijfertabel precies werd verzameld en hoe ze gereproduceerd kan worden. Bovendien preciseert de operationele definitie steeds exact welke waarden de attributen en dimensies binnen de tabel aannemen. Voor tabel \ref{tab:voorbeeldcijfertabel} luidt de operationele definitie bijvoorbeeld: ``De grootte van de wettelijke bevolking op 1 januari 2019, 0.00 uur, per Vlaamse gemeente volgens de NIS‐code‐indeling in 2019. De wettelijke bevolking omvat\ldots'' 
...
In paragraaf \ref{sec:statistiekalscijfer} zagen we dat statistieken nog heel wat andere attributen hebben naast de conceptuele en operationele definitie. Net zoals deze definities, kan informatie over deze andere attributen in een cijfertabel vaak veralgemeend worden van het niveau van het cijfer naar het niveau van de hele tabel. In de praktijk kunnen we attributen zelfs op drie verschillende niveaus toewijzen. 

Een eerste soort attributen zijn attributen op het niveau van de hele cijfertabel. Deze attributen bevatten informatie die geldt voor alle cijfers in de tabel. In Tabel \ref{tab:voorbeeldcijfertabel} is het concept jaartal een voorbeeld van zo'n attribuut aangezien alle cijfers verwijzen naar bevolkingsgroottes in jaartal 2019. Informatie over dit soort attributen hoef je uiteraard enkel op tabelniveau te bewaren en niet apart per cijfer. 
...
\input{tables/voorbeeldtabelreeks.tex}
\end{table}	
...
\begin{info}
	...
	\begin{itemize}
		...
		\item kwaliteit van een woning is een \emph{concreet en meervoudig} concept. Het bestaat uit verschillende woningkenmerken die stuk voor stuk direct observeerbaar (concreet) zijn. Ze beschrijven echter enkel allemaal samen het meervoudig concept ``kwaliteit van een woning''. Je hebt dus meerdere concrete indicatoren nodig om het volledig te vatten.
		\item Subjectieve gezondheid is een \emph{abstract en enkelvoudig} concept. Dit is een subjectief, niet rechtstreeks observeerbaar gevoel (abstract), maar het wordt in onderzoek meestal benaderd als één enkel kenmerk, bijvoorbeeld via een enkele vraag: ``Hoe is uw gezondheid over het algemeen?''. Je kan natuurlijk discussiëren of zo'n enkele vraag de nuances in subjectieve gezondheid wel volledig vat en of het niet als meervoudig concept moet worden bekeken. Opgelet, objectieve gezondheid is sowieso een meervoudig concept dat je meet via allerhande medische testen.
...
\begin{itemize}
\item Focus op statistieken als individuele cijfers is nuttig want voor elk individueel gepubliceerd cijfer moet juiste metadata worden gepubliceerd, zonder uitzondering. Meer specifiek moet voor elk cijfer duidelijk zijn wat de conceptuele en operationele definitie is, voor welke gebruikersbehoeften dit cijfer werd gepubliceerd, en hoe de kwaliteit van het cijfer wordt geëvalueerd. 
...
In de volgende hoofdstukken zullen we de begrippen uit dit hoofdstuk gebruiken om te bespreken hoe we in de praktijk ons werk kunnen inrichten als openbare statistiedienst. Hoofdstuk 2 beantwoord de vraag hoe we statistieken afbakenen? We leggen hierin het verschil uit tussen officiële, experimentele en niet-officiële statistieken, en we geven richtlijnen om tabelreeksen en cijfertabellen af te bakenen. Hoofdstuk 3 beantwoordt de vraag hoe we statistieken documenteren via metadatafiches. Hoofdstuk 4 beantwoordt de vraag hoe we statistieken beheren als datasets en hoe we structurele metadata gebruiken. Hoofdstuk 5 beantwoordt de vraag hoe we statistieken ontsluiten naar het brede publiek via databases, cijferapplicaties, cijferpagina’s en nieuwsberichten. 
