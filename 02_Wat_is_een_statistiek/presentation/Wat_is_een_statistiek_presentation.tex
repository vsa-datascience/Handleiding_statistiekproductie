...
\onslide<3->{
	\node[values,xshift=-5mm,anchor=east] (Z) at (A.west) {conceptwaarden:};
...
\onslide<2>{
	\node[criterion] at (current page.south) {Toegankelijkheid en duidelijkheid};
	\node[questions] at (current page.south) {In duidelijke en begrijpelijke vorm gepresenteerd?\\Op een passende en gebruikersvriendelijke manier gepubliceerd?};
	}
\onslide<3>{
	\node[criterion] at (current page.south) {Publiceerbaarheid};
	\node[questions] at (current page.south) {Kan de statistiek publiek worden gepubliceerd of enkel intern?};
	}
\onslide<4>{
...
\begin{frame}
\frametitle{We splitsen dimensies over kolommen en rijen,\\andere concepten staan in de tabeltitel.}
\begin{center}
\tiny
\begin{tblr}{
	width=.8\linewidth,
...
\SetCell[c=3]{l} Aantal inwoners per Vlaamse gemeenten per jaar.\\	
Gemeente & Jaartal & Cijfer \\
Aalst & 2019 & 86\,445 \\
Aalst & 2020 & 87\,382 \\
Aalter & 2019 & 28\,906 \\
Aalter & 2020 & 27\,394 \\
\ldots & \ldots & \ldots \\
Zwijndrecht & 2019 & 19\,056 \\
Zwijndrecht & 2020 & 21\,343 \\
\end{tblr}
\end{center}
\vfill\mbox{}\\
Concept 1: Parameter \\
...
\begin{frame}
\frametitle{De tabeltitel is de conceptuele definitie.}
\framesubtitle{Het bevat nog steeds alle concepten.}
\vfill
\begin{center}
``Aantal inwoners \textcolor{stonegray}{|} per gemeente \textcolor{stonegray}{|} per jaar''
\end{center}
\vfill
\begin{center}
\tiny
...
\vfill
\makebox[0pt][r]{``}De grootte van de wettelijke bevolking op 1 januari van jaren 2019 en 2020 per Vlaamse gemeente volgens de NIS‐code‐indeling van 2019. De wettelijke bevolking omvat\ldots''
\begin{flushright}
$\Rightarrow$ 300 gemeenten $\times$ 2 jaartallen = 600 cijfers
\end{flushright}
\vfill
\begin{center}
\tiny
...
\begin{frame}
\frametitle{Door een tabel te pivoteren naar lang formaat\\staan alle dimensies in de kolommen.}
\begin{tiny}
\begin{tblr}{
	width=\linewidth,
	colspec={X[c]X[c]X[c]X[c]},
...
\SetCell[c=4]{l} Aantal en percentage inwoners per geslacht in Vlaamse gemeenten in 2019.\\
Gemeente & Geslacht & Parameter & Cijfer \\
Aartselaar & man & Aantal & 7089 \\
Aartselaar & man & Percentage & 49.6 \\
Aartselaar & vrouw & Aantal & 7204 \\
Aartselaar & vrouw & Percentage & 50.4 \\
Antwerpen & man & Aantal & 262921 \\
Antwerpen & man & Percentage & 50.0 \\
Antwerpen & vrouw & Aantal & 263014 \\
Antwerpen & vrouw & Percentage & 50.0 \\
Boechout & man & Aantal & 6506 \\
\ldots & \ldots & \ldots & \ldots \\
\end{tblr}
\end{tiny}
\end{frame}
...
\begin{frame}
\hfill\includegraphics[width=\linewidth]{figures/faillissementen-van-ondernemingen-rgb.pdf}
\end{frame}


\begin{frame}
\frametitle{Er is een verschil tussen\\administratieve data en statistieken.}
\begin{scriptsize}
\begin{tblr}{
	width=\linewidth,
	colspec={X[c]X[c]},
	row{1}={bg=deepslateblue,fg=white},
	row{2}={bg=softsteelblue,fg=white},
	hlines = {mistgray},
	hline{1,2,Z} = {deepslateblue},
	vlines = {deepslateblue},
	cells = {valign = m},
	}
Administratieve ontsluiting & Statistische publicatie \\
Hoeveel faillissementen werden geregistreerd? & Hoe gezond zijn onze bedrijven? \\
Vertrekt van de databron & Vertrekt van de gebruikersvraag \\
Verzameld omwille van operationeel proces & Verzameld omwille van beleid \\
%Wat registreert administratie X? & Welk maatschappelijk fenomeen meten we? \\
%Definities impliciet en brongebonden & Definities expliciet en conceptueel afgebakend \\
%Sterk afhankelijk van administratieve processen en beleidswijzigingen & Methodologisch stabieler over de tijd \\
Beperkte vergelijkbaarheid & Internationale en regionale vergelijkbaarheid mogelijk \\
\end{tblr}
\end{scriptsize}
\end{frame}
